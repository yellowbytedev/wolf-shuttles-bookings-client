...
- A **dry-run handover envelope** with HMAC signing for safe receiver planning
...
- Produces dry-run handover envelope with HMAC-SHA256 signature
...
| Responsibility | Status |
|---------------|--------|
| v2 intake REST endpoint (`/wp-json/ws-bookings/v2/intake`) | 🟡 Scaffold exists; needs dry-run implementation |
...
- `integrity.signature`: HMAC-SHA256 hex string (or empty if no secret)
- `integrity.signed_fields`: subset of envelope fields used for signing
- `meta.preview_only`, `meta.real_handover_enabled`
...
- `scripts/run-booking-handover-preview-fixtures.php`: dry-run handover envelope assertions
- All 29 payload fixtures pass; 18 valid handover pass, 11 invalid skipped

### Handover Preview
- `inc/class-booking-payload-v2-handover-service.php`: deterministic HMAC-SHA256 signing
- Dry-run mode (`mode: "dry_run"` hardcoded)
...
php scripts/run-booking-handover-preview-fixtures.php  # 18 pass, 11 skip
...
## 14. Next Roadmap Steps

1. **Booking-site v2 receiver — dry-run** (`docs/booking-site-v2-receiver-plan.md`)
   - Implement dry-run receiver in `ws-bookings`
   - Accept marketing handover envelope, verify HMAC, validate expiry, normalise to itinerary/trip
   - Return dry-run response without creating DB records

2. **Booking-site receiver fixture runner**
   - Mirror marketing fixture runner in `ws-bookings`
   - Test envelope verification, payload adaptation, dry-run responses
...
6. **Marketing-side live handover**
   - Replace `dry_run` mode with real v2 endpoint call
   - Wire `WSB_CLIENT_V2_HANDOVER_SECRET` for production
